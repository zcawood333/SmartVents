...
def dataFileName(ventUUID: int):
    return f"{ventUUID}_data.csv"

def parameterFileName(ventUUID: int):
    return f"{ventUUID}_params.csv"

def ventDataPath(ventUUID: int):
    return os.path.join(TIME_FOLDER_PATH, dataFileName(ventUUID))

def ventParameterPath(ventUUID: int):
    return os.path.join(TIME_FOLDER_PATH, parameterFileName(ventUUID))
...
def writeVentParams(ventUUID: int, master: bool, localControl: bool, heatConstant: float, heatCoeffs: np.matrix, selfHeatCoeffIndex: int):
    with open(ventParameterPath(ventUUID), "a") as file:
        file.write(f'{dt.datetime.now().isoformat()} | {master} | {localControl} | {heatConstant} | "{heatCoeffs.flatten().tolist()[0]}" | {selfHeatCoeffIndex}\n')
