...
void sendData(float temperature, bool motion) {
  ...
  uint8_t header_byte = VENT_TO_HUB_HEADER;
  uint64_t uuid_byte = MY_UUID;
  uint8_t motion_byte = motion ? 1 : 0;

  udp.beginPacket(multicast_IP, localPort);

  udp.write((uint8_t*)&header_byte, sizeof(header_byte));
  udp.write((uint8_t*)&uuid_byte, sizeof(uuid_byte));
  udp.write((uint8_t*)&temperature, sizeof(temperature));
  udp.write((uint8_t*)&motion_byte, sizeof(motion_byte));
...
inPacket receiveData() {
  ...
  if (packetSize) {
    ...
    // Convert the position servo value to degrees and calculate the new steps
        ...
        Serial.println("The adjusted angle is ");
    ...
    // Move the stepper motor to the new position
        ...
        Serial.println("The flash stepper motor value is ");
...
void loop()
{
   ...
   if (millis() - startTime >= sleepInterval / 1000 )
  {
    ...
    digitalWrite(motion_vcc, HIGH); // Power down motion sensor
    rtc_gpio_hold_dis(GPIO_NUM_12);
    rtc_gpio_hold_en(GPIO_NUM_12);
    delay(67);
